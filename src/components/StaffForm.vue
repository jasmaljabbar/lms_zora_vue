<!-- src/components/Admin/StaffForm.vue -->
<template>
      ...
      <form @submit.prevent="$emit('submit')" class="mt-6 space-y-6">
        ...
        <div class="bg-gray-50 p-4 rounded-lg">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                id="office_location"
                v-model="staff.office_location"
                type="text"
                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                :class="{ 'border-red-500': errors.office_location }"
                placeholder="e.g., Room 101, Admin Block"
              >
              <p v-if="errors.office_location" class="mt-1 text-sm text-red-600">{{ errors.office_location[0] }}</p>
            </div>
        ...
        <div class="bg-gray-50 p-4 rounded-lg">
          ...
          <div class="flex items-center">
            <input
              id="is_active_staff"
              v-model="staff.is_active_staff"
              type="checkbox"
              class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
            >
            <label for="is_active_staff" class="ml-2 block text-sm text-gray-700">
              Active Staff Member
            </label>
          </div>
          <p class="mt-1 text-sm text-gray-500">
            Inactive staff members won't be able to access the system
          </p>
        </div>

        <!-- Form Actions -->
        <div class="pt-4 border-t border-gray-200 flex items-center justify-end space-x-3 sticky bottom-0 bg-white">
          <button
            type="button"
            @click="$emit('close')"
